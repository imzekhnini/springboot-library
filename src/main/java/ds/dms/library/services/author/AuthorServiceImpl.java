...
public class AuthorServiceImpl implements AuthorService {
    ...
    @Override
    public Page<ResponseAuthor> getAuthorsPaginated(Integer page, Integer size, String sortField, String sortDirection) {
        size = size > MAX_PAGE_SIZE ? MAX_PAGE_SIZE : size;
        Sort sort = Sort.by(Sort.Direction.fromString(sortDirection), sortField);
        Pageable pageable = PageRequest.of(page, size, sort);
        Page<Author> authors = authorRepository.findAll(pageable);
        return authors.map(authorMapper::toResponseAuthor);
    }
}
